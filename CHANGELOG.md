...
- cosmos nebula backdrop reworked at 2.5x the internal resolution (128x80 → 320x200, dither cell and feature-star coordinates/radii scaled to match, cloud noise lattice 10 → 16, starfield 90 → 350 stars) so it no longer reads as "too zoomed in"; the warp-streak burst bumped to the same 2.5x scale (128 → 320) so it doesn't look chunkier than the cosmos it reveals, now prewarmed on mount so the first real transition doesn't pay the bigger raster's generation cost mid-animation
