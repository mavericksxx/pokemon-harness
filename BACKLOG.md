...
- **garden UI crash ~04:13 GST 2026-08-29 (DISPATCHED 2026-08-29, user go-ahead after updating to v1.4.0)**: full evidence + hypotheses + questions in docs/triage/2026-08-29-garden-ui-crash.md. Short version: main process alive, no logged renderer JS error (error path provably works), no macOS crash report → leading suspect is silent WebGL/GPU context loss on the Pixi canvas; nothing listens for context-lost or render-process-gone today. Agent scope: full instrumentation list from the dossier + automatic context-loss recovery. User hasn't yet answered the two dossier questions (what the crash looked like; self-recovered vs recovered via action ~04:38) — instrumentation/recovery is correct under every hypothesis, so not blocking on them.
...
- **usage section update (requested 2026-08-29, dispatched)**: remove duplicate entries — "5h" and "session limit" are the same thing, show it once; add credit usage if the account/API exposes it.
