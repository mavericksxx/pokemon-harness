...
- **topbar view-switcher shifts on selection (reported 2026-08-29 via video, DISPATCHED)**: selecting a view changes the switcher's width — the terminal/focus button conditionally appears/disappears (5 buttons in some states, 4 in others, video-confirmed) and the selection border may add geometry — so the whole right-side cluster (usage chip, volume, theme, gear) slides. Fix: constant button set (disable, never unmount) + geometry-stable selection styling (always-present border, color-only swap).
- **usage popover redesign + per-provider selection (reported 2026-08-29, DISPATCHED)**: (1) UI "doesn't look good" — generic thin bars, no pokéharness design language; restyle as the originally-planned trainer-card: pixel-caps headers, bordered panels, pokémon HP-bar style segmented gauges. (2) Data bugs visible in screenshot: named windows AND additive `limits[]` both render, duplicating rows ("5h 75%" + "session 75%", "wk 66%" + "weekly 66%") — dedupe; long reset times shown as raw hours ("resets in 416h 57m") — humanize to days. (3) New setting: per-provider include checkboxes under settings → usage (user wants Claude only, no Codex) — excluded provider is neither polled nor displayed; master toggle off still means zero credential access.
