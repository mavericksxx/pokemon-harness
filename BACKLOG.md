...
- **per-garden backdrops (requested 2026-08-29, design artifact delivered — awaiting user's picks)**: today every garden uses the same meadow tileset. Plan: each new garden gets a different backdrop theme (rotate through the set on creation, plus a manual picker later). 8 theme concepts presented in an artifact (palettes + mockups); user picks 5+ favorites, then build = tile variants + palette swap layer over the existing tile grid. NOTE: user said "each new session" — assumed to mean each new GARDEN (sessions share one garden view); confirm.
