...
- **battle FX crash re-freezes the garden + premature completion for background agents (2026-08-29, live log evidence, DISPATCHED)**: v1.3.0's lifecycle fix works (6/6 subagents materialized, first battled) but caught a NEW crash: on battler cleanup a leftover battle effect kept the destroyed sprite and tickBattleFx threw `Cannot set properties of null (setting 'y')` — logged as "outside per-parent isolation" (the global FX pass isn't isolated), starving poof-ins and the battle queue (5 invisible battlers, battlesStarted frozen at 1 with 2 more agents finished). Second bug from same log: first battler fought/fainted ~100s after spawn while its agent still had ~8 min left — Stop-driven completion assumes Task blocks the parent turn, false for BACKGROUND/async agents. Fix: purge/guard FX on cleanup, isolate every phase of update(), and a per-subagent completion signal that works for async dispatches.

- **settings dialog redesign (reported 2026-08-29, DISPATCHED)**: the settings page is a vast empty panel — plain text nav, one lonely control floating in dead space (screenshot-confirmed on the appearance section). Restyle in the pixel trainer-menu language: bordered section cards with pixel-caps headers, nav with a proper selected frame, per-row descriptions, sane max-width content column instead of full-bleed emptiness. Keep every existing section/control and all wiring.
...
- **garden day/night cycle (requested 2026-08-29, preview artifact DISPATCHED)**: the current garden should transition sunny (day) → sunset (evening) → moonlit dark (night — not very dark, full-moon-on-the-ground feel) by real local time. HARD GATE: user wants to see exactly how it looks in an artifact BEFORE any code changes — preview artifact first, build only after approval. Likely implementation once approved: a Pixi color-grade overlay layer interpolating by local time (cheap, phase-F-friendly), not new tile art.
