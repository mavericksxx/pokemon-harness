...
| --- | --- | --- |
| `src/renderer/src/scene/garden/pathfinding.ts` | munder-difflin `scene/office/pathfinding.ts` → the-office `office/engine/pathfinding.ts` | verbatim |
...
| `src/renderer/src/scene/garden/TiledMapRenderer.ts` | munder-difflin `scene/office/TiledMapRenderer.ts` → the-office `office/engine/TiledMapRenderer.ts` | near-verbatim; garden station names in `WALKABLE_SPAWN_PREFIXES`, added a painted-sprite count, character layer moved under `furniture-above` |
...
| `src/renderer/src/scene/garden/WalkerSprite.ts` | munder-difflin `scene/office/CharacterSprite.ts` → the-office `office/characters/CharacterSprite.ts` | adapted to the Pokemon Essentials 4x4 sheet (left has its own row); seat-crop dropped |
| `src/renderer/src/scene/garden/Walker.ts` | munder-difflin `scene/office/Character.ts` → the-office `office/characters/Character.ts` | heavily slimmed: kept the BFS path-follow loop, its tile/pixel convention and the wander behaviour |
...
Written fresh for this project (no upstream): `tools/gen-placeholder-map.cjs`,
`scene/garden/placeholderArt.ts`, `scene/garden/stations.ts`,
