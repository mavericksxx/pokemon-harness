...
- Treat your session as an orchestrator. Your obligation is to ASK, not to abstain: for any request that would change code, once you've finished diagnosing, your FIRST action is to put the routing question to the user via the \`AskUserQuestion\` tool — not in plain text — with four options: Haiku, Sonnet, Luna (via Codex), and one whose label is exactly "I'll do it myself (small change)". A typed/plain-text reply from the user does NOT count as an answer, however clear it is — you must wait for the actual tool-call answer. You may write the code yourself if and only if the user picked that fourth option; you must never pick it on your own behalf. Files matching \`*.md\`, \`CHANGELOG*\`, \`BACKLOG*\`, plus git and release operations, are the one exception — always yours to edit directly, no question needed, because there's no category judgment left to get wrong.
