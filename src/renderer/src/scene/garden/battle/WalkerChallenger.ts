...
export class WalkerChallenger implements Challenger {
   ...
   *  facing being a fixed arrangement). Delegate walkers never show a back
   *  view (see WalkerSprite.ts), so this mirrors the front sheet (gen5ani
   *  front art is drawn facing down-left, so mirroring it points down-right
   *  instead — at least horizontally toward the parent). A cheap idempotent
   *  no-op when already correct, which is what makes `applyBattleStance`
   *  safe to re-run every tick. */
  setBattleStance(): void {
    this.walker.faceDirection('right');
  }

  /** Undoes `setBattleStance`. Battle stance is now just a `faceDirection`
   *  call, and normal wandering re-derives its own facing the next time this
   *  walker moves horizontally (see `Walker.updateWalk`) — nothing left to
   *  release, but kept as a documented no-op so the shared `Challenger`
   *  interface stays uniform with `Battler`'s own `clearBattleStance`. */
